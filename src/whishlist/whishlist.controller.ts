import {
  Controller,
  Get,
  Post,
  Body,
  Delete,
  UseGuards,
  Req,
} from '@nestjs/common';
import { ApiTags, ApiBearerAuth, ApiOperation, ApiResponse, ApiBody } from '@nestjs/swagger';
import { WhishlistService } from './whishlist.service';
import { CreateWhishlistDto } from './dto/create-whishlist.dto';
import { JwtAuthGuard } from 'src/auth/guards/jwt.auth.guard';

@ApiTags('Wishlist')
@ApiBearerAuth("access-token") 
@Controller('whishlist')
export class WhishlistController {
  constructor(private readonly whishlistService: WhishlistService) {}

  @Post()
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Add product to wishlist' })
  @ApiResponse({ status: 201, description: 'Product added to wishlist successfully' })
  @ApiBody({ type: CreateWhishlistDto })
  create(@Req() request: Request, @Body() createWhishlistDto: CreateWhishlistDto) {
    const userId = (request as any).user.id;
    return this.whishlistService.create(userId, createWhishlistDto);
  }

  @Get()
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Get all wishlist items of the user' })
  @ApiResponse({ status: 200, description: 'Wishlist fetched successfully' })
  findAll(@Req() request: Request) {
    const userId = (request as any).user.id;
    return this.whishlistService.findAll(userId);
  }

  @Delete()
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Remove a product from the wishlist' })
  @ApiResponse({ status: 200, description: 'Product removed from wishlist successfully' })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        productId: { type: 'string', example: '66298f7e1c3fdf3e88b6e2f9' },
      },
      required: ['productId'],
    },
  })
  removeFromWhishlist(@Req() request: Request, @Body() body: { productId: string }) {
    const userId = (request as any).user.id;
    return this.whishlistService.removeProductFromWhislist(userId, body.productId);
  }

  @Delete('/whishlist-delete')
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Delete entire wishlist of the user' })
  @ApiResponse({ status: 200, description: 'Wishlist deleted successfully' })
  deleteWhishlist(@Req() request: Request) {
    const userId = (request as any).user.id;
    return this.whishlistService.deleteWhishlist(userId);
  }
}
